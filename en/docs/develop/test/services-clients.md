...
Test your services end-to-end by invoking real endpoints from within a test function, and test your client code by mocking the responses it depends on. The same test infrastructure from [Unit Testing](unit.md) applies — create a test function, open it in the flow diagram, and build your test logic from there.
...
1. Create a test function as described in [Unit Testing](unit.md) and open it in the flow diagram view.
...
6. Add assertion nodes from the **Test** category to verify the response. See [Unit Testing — Assertions](unit.md#assertions) for available assertion nodes.
...
- [Unit Testing](unit.md) — create test functions with assertions
- [Mocking](mocking.md) — complete guide to object and function mocking
- [Execute Tests](execute.md) — run tests and view results
